...
# GAMS is optional - GDX loading only available when GAMS.jl is installed
const HAS_GAMS = try
	@eval import GAMS
	true
catch
...
function load(path::AbstractString, model::AbstractModel, renames::Pair...; kwargs...)
	...
	if endswith(lowercase(path), ".gdx")
		if !HAS_GAMS
			error("Loading GDX files requires GAMS.jl to be installed. Install with: using Pkg; Pkg.add(\"GAMS\")")
		end
...
# GDX loading function - only defined when GAMS is available
if HAS_GAMS
